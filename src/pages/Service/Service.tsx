...
const Service = () => {
  ...
  const [token, setToken] = useState<string | null>(localStorage.getItem("token")); // Stocker le token JWT
              ...
              services.map((service) => (
                <motion.div key={service.id} variants={itemVariants} whileHover={{ scale: 1.02, transition: { duration: 0.2 } }} className="bg-white/10 backdrop-blur-sm rounded-xl overflow-hidden border border-white/20">
                  <div className="p-6">
                    ...
                    <button onClick={() => (window.location.href = `/service/${service.id}`)} className="w-full bg-white text-[#197277] font-semibold py-3 px-4 rounded-lg hover:bg-white/90 transition-colors duration-200">
                      En savoir plus
                    </button>
                  </div>
                </motion.div>
